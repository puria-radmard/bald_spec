...
class SimulatedDataLoader(ActiveLearningDataLoaderBase):
    ...
    """
    For each input_data:
        sample a theta from true_hypothesis_space's prior,
        evaluate f with reg_model
        add noise to it with noise_model
    
    Obviously, if fitting a model to this, you should have true_hypothesis_space's ("prior") distribution 
        be different to the prior you start with in the fitting hypothesis space

        parameter_sampler is model agnostic

        reg_model should be the same if you're directly comparing the fitted posterior with true_hypothesis_space's prior
    ...
    def __init__(
        ...
        true_hypothesis_space: _H,      # Along with parameter_sampler, needed to sample some theta This should be diffe
        ...
        labels = self.generate_data(input_data, num_hypothesis, true_hypothesis_space, reg_model, noise_model)
        ...
        super().__init__(input_data, labels, training_indices, training_batch_size, querying_batch_size)


    def generate_data(
        self, input_data: _T, num_hypothesis: int, true_hypothesis_space: _H, reg_model: _NL, noise_model: _NM
    ) -> _T:

        # Sample a few hypotheses and evaluate at data
        thetas = true_hypothesis_space.sample_from_prior(num_hypothesis)     # [I, dim(H)]
